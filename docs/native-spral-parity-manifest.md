...
| Line search, `alpha_for_y`, filter, watchdog, SOC, restoration | `IpFilterLSAcceptor.cpp::CalculateAlphaMin`, `IpBacktrackingLineSearch.cpp::DoBacktrackingLineSearch`, IPOPT `Algorithm/` source for the corresponding component | `SpralSrc` parity mirrors IPOPT's filter alpha-min formula and first-trial backtracking guard. Remaining line-search behavior should change only when direction/KKT parity evidence says the divergence has moved above the linear solve. |
