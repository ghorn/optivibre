...
| Inequality slack internal sign convention | `IpIpoptCalculatedQuantities.cpp::curr_slack_s_U`, `curr_grad_lag_s`, `curr_sigma_s`; `IpDefaultIterateInitializer.cpp::push_variables`; `IpPDFullSpaceSolver.cpp::SolveOnce` | NLIP now stores inequality slacks in IPOPT's upper-bound internal sign for the shifted zero-bound row (`g_relaxed(x) - s = 0`, `s <= 0`) and converts explicitly to upper-slack distance for barrier/complementarity calculations. The remaining accepted deviation is that NLIP shifts the normalized inequality function by the relaxation amount instead of carrying IPOPT's relaxed `d_U` bound vector; this must be removed before claiming full internal-slack state parity. |
