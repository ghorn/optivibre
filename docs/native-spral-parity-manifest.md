...
| P6 | `IpBacktrackingLineSearch` fallback, restoration, watchdog, and soft-restoration edge paths | Coverage active-watch backlog; non-default `magic_steps`, min-dual-infeas `alpha_for_y` variants, tiny-step eval-error fallback, and low-`tiny_step_y_tol` dual-threshold behavior are now covered |
