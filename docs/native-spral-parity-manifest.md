...
| --- | --- | --- |
| `IpDefaultIterateInitializer.cpp::push_variables` and multiplier initialization | Covered by max-iter-0 state comparisons and scalar push tests. | New max-iter-0 mismatch in x, slack, y, z/v, damping, objective, or residual components. |
...
| `IpIpoptAlg.cpp::correct_bound_multiplier` | Covered for the active lane; variable-bound correction vectors are source-backed. | First accepted-state mismatch in variable-bound multipliers or correction-vector diagnostics. |
| `IpMonotoneMuUpdate.cpp` and line-search reset on mu change | Covered by monotone-mu update tests and source-backed reset semantics. | Barrier parameter, tau, filter reset, watchdog reset, or tiny-step flag diverges before direction/state drift. |
| `IpFilterLSAcceptor.cpp`, watchdog, SOC, tiny-step, and restoration | Filter local/global acceptability, `CalculateAlphaMin`, first-trial SOC gating, filter update, and tiny-step source branches are checked. The active glider SOC branch is source-backed by `TrySecondOrderCorrection` and `DenseVector::AddOneVector`; the full source-SPRAL run now has matching accepted directions, alphas, and accepted states through IPOPT's 152 accepted steps. | Matching accepted state and directions immediately before a divergent accept/reject, watchdog, SOC, tiny-step, or restoration decision; do not use restoration or SOC as a repair while direction/state drift is already present. |
