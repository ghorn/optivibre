...
| P6 | `IpBacktrackingLineSearch` fallback, restoration, watchdog, and soft-restoration edge paths | Coverage active-watch backlog; non-default `magic_steps`, min-dual-infeas `alpha_for_y` variants, and tiny-step eval-error fallback are now covered |
