...
| P5 | `IpPDFullSpaceSolver` refinement, failure, and quality-retry branches | Refinement option surface, failed-refinement decision order, and quality-function `improve_solution=true` seeded refinement are implemented and pinned; remaining coverage active-watch rows need live failed-solve, quality-retry, pretend-singular marker, and accept-current marker witnesses |
| P6 | `IpBacktrackingLineSearch` fallback, restoration, watchdog, and soft-restoration edge paths | Coverage active-watch backlog; non-default `magic_steps` and min-dual-infeas `alpha_for_y` variants are now covered |
...
| P8 | `IpDefaultIterateInitializer` equality-multiplier, failure, switch, and low-push branches | Coverage active-watch backlog; zero `bound_push` is option-rejected for the main initializer |
