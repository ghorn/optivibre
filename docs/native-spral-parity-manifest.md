...
| `IpFilterLSAcceptor.cpp`, watchdog, SOC, tiny-step, and restoration | Filter local/global acceptability, `CalculateAlphaMin`, first-trial SOC gating, filter update, watchdog arming, and tiny-step source branches are checked. The active glider SOC branch is source-backed by `TrySecondOrderCorrection` and `DenseVector::AddOneVector`; focused option tests exercise SOC-disabled, watchdog-disabled, a watchdog-trigger profile with matching accepted traces, tiny-step-disabled, forced tiny-step acceptance, non-default `alpha_red_factor`, and monotone-fast-decrease-disabled paths. IPOPT `TryCorrector` remains unreachable under `corrector_type=none`. The full source-SPRAL run now has matching accepted directions, alphas, and accepted states through IPOPT's 152 accepted steps. | Matching accepted state and directions immediately before a divergent accept/reject, watchdog, SOC, tiny-step, or restoration decision; do not use restoration or SOC as a repair while direction/state drift is already present. |
