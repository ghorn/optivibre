...
* `threading-serial` to build without OpenBLAS internal worker threads (enabled by default),
* `threading-pthread` to build OpenBLAS with its pthread backend,
* `threading-openmp` to build OpenBLAS with its OpenMP backend.

Exactly one threading feature must be enabled. `threading-openmp` forwards to
OpenBLAS `USE_THREAD=1` and `USE_OPENMP=1`; `threading-pthread` forwards to
`USE_THREAD=1`; `threading-serial` leaves both disabled. Set
`OPENBLAS_NUM_PARALLEL` to populate OpenBLAS `NUM_PARALLEL` for OpenMP builds
that may receive concurrent calls from independent OpenMP regions.
