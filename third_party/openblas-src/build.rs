...
fn threading_features() -> usize {
    [
        feature_enabled("threading-serial"),
        feature_enabled("threading-pthread"),
        feature_enabled("threading-openmp"),
    ]
    .iter()
...
fn build() {
    ...
    assert!(
        threading_features() == 1,
        "exactly one openblas-src threading feature must be enabled: \
         threading-serial, threading-pthread, or threading-openmp"
    );
    ...
    if feature_enabled("threading-pthread") {
        cfg.use_thread = true;
    } else if feature_enabled("threading-openmp") {
        cfg.use_thread = true;
        cfg.use_openmp = true;
        if let Ok(num_parallel) = env::var("OPENBLAS_NUM_PARALLEL") {
            // openblas-build does not expose NUM_PARALLEL directly yet, but
            // OpenBLAS make reads it from the environment.
            env::set_var("NUM_PARALLEL", num_parallel);
        }
    }
