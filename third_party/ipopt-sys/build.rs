...
fn build_cnlp(ipopt_include_paths: &[PathBuf]) -> PathBuf {
    ...
    for (index, path) in ipopt_include_paths.iter().enumerate() {
        let candidate = if path.join("coin").join("IpIpoptApplication.hpp").exists() {
            path.clone()
        } else if path.join("IpIpoptApplication.hpp").exists() {
            let compat_root = out_dir.join(format!("ipopt-include-compat-{index}"));
            let compat_coin_dir = compat_root.join("coin");
            let _ = fs::remove_file(&compat_coin_dir);
            let _ = fs::remove_dir_all(&compat_coin_dir);
            fs::create_dir_all(&compat_root).unwrap();
            #[cfg(target_family = "unix")]
            std::os::unix::fs::symlink(path, &compat_coin_dir).unwrap();
            #[cfg(not(target_family = "unix"))]
            fs::create_dir_all(&compat_coin_dir).unwrap();
            compat_root
        } else if path
            .parent()
            .is_some_and(|parent| parent.join("coin").join("IpIpoptApplication.hpp").exists())
        {
            ...
            let compat_root = out_dir.join(format!("ipopt-include-compat-{index}"));
            let compat_coin_dir = compat_root.join("coin");
            let actual_coin_dir = path.parent().unwrap().join("coin-or");
            let _ = fs::remove_file(&compat_coin_dir);
            let _ = fs::remove_dir_all(&compat_coin_dir);
            fs::create_dir_all(&compat_root).unwrap();
            #[cfg(target_family = "unix")]
            std::os::unix::fs::symlink(actual_coin_dir, &compat_coin_dir).unwrap();
            #[cfg(not(target_family = "unix"))]
            fs::create_dir_all(&compat_coin_dir).unwrap();
            compat_root
        } else {
        ...
        normalized_include_paths.push(candidate);
    }
    let ipopt_include_dirs = normalized_include_paths
        .iter()
