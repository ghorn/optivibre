...
fn blank_kite_diag<T: Scalar>() -> KiteDiagnostics<T> {
    KiteDiagnostics {
        ...
        rabbit_distance: T::zero(),
        rabbit_target_distance: T::zero(),
        rabbit_bearing_y: T::zero(),
        rabbit_vector_b: zero_vec(),
        rabbit_target_n: zero_vec(),
        lateral_outer_mode: T::zero(),
        forward_frame_heading: T::zero(),
        forward_lane_y: T::zero(),
        forward_cross_track_error: T::zero(),
        forward_neighbor_prev_y_f: T::zero(),
        forward_neighbor_next_y_f: T::zero(),
        forward_formation_error: T::zero(),
        forward_formation_spacing: T::zero(),
        forward_lateral_offset: T::zero(),
        forward_lane_point_n: zero_vec(),
        forward_formation_error_tip_n: zero_vec(),
        orbit_radius: T::zero(),
        ...
        aileron_trim: T::zero(),
        aileron_roll_p: T::zero(),
        aileron_roll_d: T::zero(),
        rudder_trim: T::zero(),
        rudder_beta_p: T::zero(),
        rudder_rate_d: T::zero(),
        rudder_world_z_p: T::zero(),
        elevator_trim: T::zero(),
        elevator_pitch_p: T::zero(),
        elevator_pitch_d: T::zero(),
        elevator_alpha_protection: T::zero(),
        motor_torque_trim: T::zero(),
        motor_torque_p: T::zero(),
        motor_torque_i: T::zero(),
        motor_force: T::zero(),
        motor_power: T::zero(),
...
fn spring_energy<T: Scalar>(ea: T, natural_length: T, current_length: T) -> T {
    let strain = (current_length / natural_length - T::one()).max(T::zero());
...
fn compute_tether<T: Scalar, const N: usize>(
    ...
    let mut upper_tether_forces = [zero_vec(); N];
    let mut lower_tether_forces = [zero_vec(); N];
    ...
    for index in 0..N {
        ...
        let total_force = add(&lower_tether_forces[index], &upper_tether_forces[index]);
        let total_force = add(
    ...
    let force_on_top_n = upper_tether_forces
        .last()
        .map(|force| scale(force, -T::one()))
        .unwrap_or_else(zero_vec);
    let force_on_bottom_n = lower_tether_forces
        .first()
        .map(|force| scale(force, -T::one()))
        .unwrap_or_else(zero_vec);
...
mod tests {
    ...
    #[test]
    fn compressed_tether_links_are_slack() {
        let params = TetherParams {
            natural_length: 20.0,
            total_mass: 2.0,
            ea: 1000.0,
        ...
        let top = TetherNode {
            pos_n: Vector3::new(0.0, 8.0, 0.0),
            vel_n: Vector3::zeros(),
        };
        let nodes = [TetherNode {
            pos_n: Vector3::new(0.0, 4.0, 0.0),
            vel_n: Vector3::zeros(),
        }];

        let output = compute_tether(&bottom, &top, &params, &Vector3::zeros(), 1.0, 0.0, &nodes);
        let link_tensions = compute_tether_link_tensions(&bottom, &top, &params, &nodes);

        assert_vec_close(&output.node_ddt[0].vel_n, [0.0, 0.0, 0.0], 1.0e-12);
        assert_vec_close(&output.force_on_top_n, [0.0, 0.0, 0.0], 1.0e-12);
        assert_vec_close(&output.force_on_bottom_n, [0.0, 0.0, 0.0], 1.0e-12);
