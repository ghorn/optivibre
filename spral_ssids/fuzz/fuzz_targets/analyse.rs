...
fuzz_target!(|data: &[u8]| {
    ...
    if let Ok(matrix) = spral_ssids::SymmetricCscMatrix::new(dimension, &col_ptrs, &row_indices, None)
    {
        let _ = spral_ssids::analyse(matrix, &spral_ssids::SsidsOptions::default());
    }
});
